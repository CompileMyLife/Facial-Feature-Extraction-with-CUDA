...
//printf("sum3: %d\n",sum);
  ...
  /* Only print for Candidate (2015, 863), Stage 0, Feature 0 */
  if (p_offset == 2015 + (863 * cascade->sum.width) && tree_index == 0) {
    printf("[CPU DEBUG] Candidate (2015,863), Stage 0, Feature 0: sum1 = %d, sum2 = %d, sum3 = %d, final_sum = %d\n",
           sum1, sum2, sum3, final_sum);
  }
...
int runCascadeClassifier( myCascade* _cascade, MyPoint pt, int start_stage )
{
  ...
  for( i = start_stage; i < cascade->n_stages; i++ )
      ...
      for( j = 0; j < stages_array[i]; j++ )
	{
	   ...
	   **************************************************/
    //printf("[CPU DEBUG] Candidate (%d,%d), Stage %d, Feature %d: Starting evaluation\n",
       //pt.x, pt.y, i, haar_counter);
